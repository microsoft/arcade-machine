...
export class FocusService {
  ...
  public setRoot(root: HTMLElement, scrollSpeed: number) {
    ...
    for (let el = this.selected; el && el !== root; el = el.parentElement) {
      if (!el) {
  ...
  public selectNode(next: HTMLElement, scrollSpeed: number) {
    ...
    if (attached) {
      ...
      for (let el = selected; el && el !== common && el; el = el.parentElement) {
        el.classList.remove(cssClass.selected);
        this.triggerFocusChange(el, null);
      }
      for (let el = next; el && el !== common && el; el = el.parentElement) {
        el.classList.add(cssClass.selected);
        this.triggerFocusChange(el, next);
        this.parents.push(el);
      }
      for (let el = common; el && el !== this.root && el; el = el.parentElement) {
        this.triggerFocusChange(el, next);
        this.parents.push(el);
      }
    } else {
      ...
      for (let el = next; el && el !== this.root && el; el = el.parentElement) {
        el.classList.add(cssClass.selected);
        this.triggerFocusChange(el, next);
        this.parents.push(el);
      }
  ...
  private rescroll(el: HTMLElement, scrollSpeed: number, container: HTMLElement) {
    ...
    for (let parent = el.parentElement; parent && parent !== container.parentElement && parent; parent = parent.parentElement) {
